...
#include "app_way_planner_core.h"
...
namespace WayPlannerNS
{
	...
	way_planner_core::way_planner_core()
	{
		...
		{
			m_bhavestart = m_bhavegoal = false;
			m_GoalsPos.resize(2);

			sub_start_pose = nh.subscribe("/start", 1, &way_planner_core::callbackGetStartPose, this);
		...
		while (ros::ok())
		{
			...
			if (m_GoalsPos.size() > 1)
			{
				...
				if (m_bhavegoal&&m_bhavestart)
				{
					...
					m_bhavegoal = m_bhavestart = false;
				}
